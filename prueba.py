...
def pdf_a_cuerpo(ruta_pdf, carpeta_slug, slug):
    ...
    falló porque el ancho de columna cambia de una tabla a otra, y un
    solo error de medición arruinaba TODAS las tablas siguientes). Ahora
    se separan por COLOR + NEGRILLA: en este documento, toda etiqueta de
    la columna izquierda está siempre en azul institucional y en negrilla;
    el contenido de la derecha nunca tiene ese estilo. Esa señal es
    constante en todo el PDF, sin importar qué tan ancha sea cada tabla.
    """
    ...
    def es_label_celda(sp):
        # sp = (y0, x0, sz, bold, rgb, text)
        return sp[3] and _cerca(sp[4], _AZUL, 30)
    ...
    for num_pag, pagina in enumerate(doc):
        ...
        for linea in lineas_logicas:
            ...
            if all(_cerca(sp[4], _BLANCO, 15) for sp in linea):
                if en_tabla:
                    continue  # repetición decorativa al cruzar de página
                flush_h2()
                flush_h3()
                en_tabla = True
                y_fila_actual = None
            ...
            if en_tabla:
                ...
                spans_izq = [sp for sp in linea if es_label_celda(sp)]
                spans_der = [sp for sp in linea if not es_label_celda(sp)]
